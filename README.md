...
| 外部无源蜂鸣器页里点 **开 / 关** | 控制 PA6（GPIOA:6）上的无源蜂鸣器（方波 ≈500 Hz）。该页**没有心跳** —— 它不归内核 leds-gpio 管，没有 `trigger` 文件 |
| 调试页末位 **【返回】** | 回首页 |
...
- [ ] **外部无源蜂鸣器还没上板实测**：PA6 的空闲是 `gpioinfo` 实测的，但"接对了线 + 真能响"
      还没验；现在的音调是主循环翻转出来的方波（≈500 Hz），要更高/更准得走内核 PWM
