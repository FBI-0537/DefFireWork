...
├──────────────────────────────────────────┤   ├──────────────────────────────────────────┤
│ ┌─────────┐┌─────────┐┌─────────┐        │   │┌───────┐┌───────┐┌───────┐┌───────┐       │ ← 底栏: 首页 2 行 × 3 列,
│ │ LED 调试 ││蜂鸣器 调试││  功能 8  │        │   ││LED 开 ││LED 关 ││LED 心跳││  返回 │       │   调试页 1 行 × 4 列。
│ └─────────┘└─────────┘└─────────┘        │   │└───────┘└───────┘└───────┘└───────┘       │   高度按最多的那页算,
│ ┌─────────┐┌─────────┐┌─────────┐        │   │                                          │   所以调试页下面空着
│ │  功能 A  ││  功能 B  ││  功能 C  │        │   │                                          │   (换来切页时内容区
│ └─────────┘└─────────┘└─────────┘        │   │                                          │    不上下跳)
└──────────────────────────────────────────┘   └──────────────────────────────────────────┘
```
...
| 调试页 | 开 / 关 / 心跳 / 返回 | (10, 408) / (263, 408) / (516, 408) / (769, 408) | 243×78 |
...
- 顶栏高 66 px（屏高 11%）；底栏高 202 px = 2×78 + 行距 26 + 上下边距 10×2；
  ...
  所以调试页（只有 1 行）下面会空出一段 —— 换来的是切页时内容区不上下跳。
