...
├──────────────────────────────────────────┤   ├──────────────────────────────────────────┤
│ ┌─────────┐┌─────────┐┌─────────┐        │   │                                          │ ← 底栏: 首页 2 行 × 3 列,
│ │ LED 调试 ││蜂鸣器 调试││  功能 8  │        │   │┌───────┐┌───────┐┌───────┐┌───────┐       │   调试页 1 行 × 4 列。
...
| 调试页末位 **【返回】** | 回首页 |
| 点【功能 8】/【功能 A】/【功能 B】/【功能 C】 | 占位（暗描边），打印 `(占位, 功能待定)` |
| 按 `q` / `Esc` | 退出程序（三页都一样） |
...
| 首页 第 1 行 | LED 调试 / 蜂鸣器 调试 / 功能 8 | (10, 408) / (348, 408) / (686, 408) | 328×78 |
| 首页 第 2 行 | 功能 A / 功能 B / 功能 C（占位） | (10, 512) / (348, 512) / (686, 512) | 328×78 |
| 调试页 | 开 / 关 / 心跳 / 返回 | (10, 460) / (263, 460) / (516, 460) / (769, 460) | 243×78 |
...
- [ ] 填充首页那 4 个占位按钮（`功能 8` / `功能 A` / `功能 B` / `功能 C`）—— 要么接上真实
      功能，要么换成真正的入口（其余按钮都已接上板载设备）
