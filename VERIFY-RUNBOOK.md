...
已连接 X server
  ...
  布局   : 深色工业风 —— 顶栏(品牌/状态灯/退出) + 内容区 + 分页底栏 (3 页, 首页 3 个按钮)

交互:
  点右上角【退出】   退出程序
  首页点【LED 调试】/【蜂鸣器 调试】  进对应调试页, 页内末位【返回】回首页
  调试页里点按钮     单独控制板载 LED / 蜂鸣器 (未绑定的打印占位提示)
  按 q / Esc        退出程序 (每一页都一样)
```
...
```
首页 (2 行 × 3 列) : LED 调试 ｜ 蜂鸣器 调试 ｜ 功能 8
                     功能 A ｜ 功能 B ｜ 功能 C          ← 后 4 个都是占位
LED 调试页         : LED 开 ｜ LED 关 ｜ LED 心跳 ｜ 返回
蜂鸣器调试页       : 蜂鸣器开 ｜ 蜂鸣器关 ｜ 蜂鸣器心跳 ｜ 返回
```

底栏高度三页一致（按最多的首页 2 行算）—— **调试页只有 1 行，下面会空出一段**，
这是有意的（换来切页时内容区不上下跳），不是渲染问题。
...
| LED 调试页 | 返回 | `… → 返回首页`，随后 `切换到 首页 (底栏 3 个按钮)` | 底栏换成首页 3 个按钮 | — |
...
| 蜂鸣器调试页 | 返回 | `… → 返回首页` | 底栏换成首页 3 个按钮 | — |
| （首页） | 功能 8 / A / B / C（4 个占位） | `… → (占位, 功能待定)` | 无（按钮带暗描边标识占位） | — |
...
- [板] 按钮：
  ...
  功能 8/A/B/C ___  退出 ___      （通过 / 失败，失败写控制台原文）
  + 心跳→开 顺序回归：常亮 / 仍闪
