...
已连接 X server
  ...
  布局   : 深色工业风 —— 顶栏(品牌/状态灯/退出) + 内容区 + 底栏 7 个功能按钮 (2 行)

交互:
  点右上角【退出】   退出程序
  点底部功能按钮     控制板载 LED / 蜂鸣器 (未绑定的打印占位提示)
  按 q / Esc        退出程序
```
...
界面布局：顶栏右侧【退出】；底栏 2 行 —— 上排 `LED 开 / LED 关 / LED 心跳 / 蜂鸣器开`，
下排 `蜂鸣器关 / 蜂鸣器心跳 / 功能 8`。
...
| 点击 | 控制台应打印 | 硬件应表现 | sysfs 旁证 |
|---|---|---|---|
| LED 开 | `点击【LED 开】坐标 (x,y) → LED 已点亮` | LED 常亮 | `sys-led/brightness` = `1`，`trigger` = `none` |
| LED 关 | `… → LED 已熄灭` | LED 灭 | `brightness` = `0` |
| LED 心跳 | `… → LED 心跳已开启` | LED 周期性闪 | `sys-led/trigger` = `heartbeat` |
| 蜂鸣器开 | `… → 蜂鸣器已响` | 持续响 | `beep/brightness` = `1`，`trigger` = `none` |
| 蜂鸣器关 | `… → 蜂鸣器已停` | 停 | `brightness` = `0` |
| 蜂鸣器心跳 | `… → 蜂鸣器心跳已开启` | 间歇响 | `beep/trigger` = `heartbeat` |
| 功能 8 | `… → (占位, 功能待定)` | 无（按钮带暗描边标识占位） | — |
| 退出（右上） | `点击【退出】按钮, 退出` → 再打印 `退出` | 窗口消失、进程结束 | 退出码 0 |
...
**额外必测一组（回归 `0b8d30d` 那个修复）**：先点【LED 心跳】，**再**点【LED 开】。
...
- [板] 按钮：
  ...
  功能 8 ___  退出 ___      （通过 / 失败，失败写控制台原文）
  + 心跳→开 顺序回归：常亮 / 仍闪
...
| 底栏只有 7 个按钮，代码里却是 `kNumButtons = 8` | 第 8 个是顶栏的【退出】（`kButtons[0]`） |
| 底栏两行宽度不等（下排 3 个比上排 4 个宽） | 已知外观问题，待办 #7 |
